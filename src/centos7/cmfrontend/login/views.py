...
def regist(request):
    ...
    if request.method == 'POST':
        ...
        uf = UserForm()
    return render(request, 'login/regist.html', {'uf':uf,'myhostip':myhostip}, context_instance=RequestContext(request))
...
def login(request):
    ...
    if request.method == 'POST':
        ...
        uf = UserForm()
    return render(request, 'login/login.html', {'uf':uf,'myhostip':myhostip}, context_instance=RequestContext(request))

#登陆成功
def main(request):
    myhostip = get_host_ip()
    user = request.COOKIES.get('user','')
    return render(request, 'main/main.html' ,{'user':user,'myhostip':myhostip})
